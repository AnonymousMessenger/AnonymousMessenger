...
public class DbHelper {
    ...
    public static List<String[]> getContactsList(DxApplication app){
        ...
        File databaseFile = new File(app.getFilesDir(), "demo.db");
        SQLiteDatabase database = SQLiteDatabase.openOrCreateDatabase(databaseFile,
                app.getAccount().getPassword(),
                null);
    ...
    public static boolean saveMessage(UserMessage msg, DxApplication app, String conversation, boolean received){
        ...
        SQLiteDatabase database = SQLiteDatabase.openOrCreateDatabase(databaseFile,
                ...
                null);
        database.execSQL(DbHelper.getMessageTableSqlCreate());
//        android.database.Cursor c=database.rawQuery("SELECT * FROM message WHERE conversation=?",
